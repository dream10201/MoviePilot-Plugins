...
def decode_action(callback_text: str) -> Tuple[Optional[str], Optional[Action]]:
    ...
    try:
        ...
        action_parts = {p.split(':', 1)[0]: p.split(':', 1)[1] for p in action_str.split('|') if ':' in p}
        ...
        return session_id, Action(command=command_def.name, value=action_parts.get("v"), view=view_name)
