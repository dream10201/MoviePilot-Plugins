...
class BaseSessionManager(Generic[TSession]):
    ...
    def get_or_create(self, event_data: Dict[str, Any], plugin_id: str) -> TSession:
        ...
        with self._lock:
            ...
            if not session or (time.time() - session.last_active > self.TIMEOUT_SECONDS):
    ...
    def cleanup(self):
        ...
        with self._lock:
            # 找出所有过期的会话
            expired_keys = [sid for sid, s in self._sessions.items() if now - s.last_active > self.TIMEOUT_SECONDS]
